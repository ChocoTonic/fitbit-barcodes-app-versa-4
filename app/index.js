...
function isEAN13(str) {
  ...
  if (/^[0-9]+$/.test(str)) {
    ...
    if (len === 12 && getCheckDigit(str.substr(0, 11)) === str.charAt(11) * 1)
      return true;
    if (len === 13 && getCheckDigit(str.substr(0, 12)) === str.charAt(12) * 1)
      return true;
  }
  return false;
}
...
function toEAN13(str) {
  ...
  for (let i = 1; i < 7; i++) {
    arr.push(
      parseInt(((ptn & 1) === 1 ? G : L).substr(str.charAt(i) * 2, 2), 16),
    );
    ptn >>= 1;
  }
  arr.push(10); //Middle
